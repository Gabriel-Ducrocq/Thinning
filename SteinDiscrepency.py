...
weights = np.genfromtxt(arguments.indexes_path, delimiter=",")
...
vfk0 = make_imq(chain, chain, pre='med')
...
if "SMPCOV" in indexes.keys() or "SCLMED" in indexes.keys() or "MED" in indexes.keys():
    ...
    for method in indexes.keys():
        ...
        thin_mat = kmat(chain[indexes[method]["indexes"]], chain[indexes[method]["indexes"]], vfk0)
    ...
    for i in range(all_selected):
        ...
        thin_mat = kmat(chain[np.abs(selected) == 1], chain[np.abs(selected) == 1], vfk0)
